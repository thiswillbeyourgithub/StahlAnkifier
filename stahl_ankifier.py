...
def _output_filename(format: str) -> str:
    """Return the .apkg filename for a card format (basic keeps the bare name)."""
    if format == "basic":
        return f"stahl_drugs_v{VERSION}.apkg"
    return f"stahl_drugs_v{VERSION}_{format}.apkg"
...
def _write_deck(
    ...
    """
    Build a genanki deck from the parsed cards in one format and write it.

    The cards and media are produced once by parse_pdf (format-independent);
    this turns them into a model/deck of the requested format and writes the
    package to output_file. Building several formats therefore reuses a single
    parse instead of re-reading the PDF per format.
    """
...
def parse_pdf(
    pdf_path: str,
    format: Literal[
        "basic", "singlecloze", "onecloze", "multicloze", "all", "json"
    ] = "basic",
    ...
    format : Literal["basic", "singlecloze", "onecloze", "multicloze", "all", "json"], optional
        Card format to use:
        ...
        - "all": Build every apkg above from a single parse, plus the JSON dump
        - "json": Only write the JSON dump (no apkg is built)
        The PDF is parsed once regardless of format; "all" reuses that single
        parse to emit every deck rather than re-reading the PDF per format.
    ...
    dump_json : str | None, optional
        Path to write the list of extracted cards as JSON before building any
        Anki package. If None (default), no JSON is written, except that the
        "all" and "json" formats default this to "cards_dump.json".
    ...
    # An explicit --dump-json path always wins; the "all" and "json" formats
    # default to cards_dump.json so a single call produces it alongside the decks.
    dump_target = dump_json or (
        "cards_dump.json" if format in ("all", "json") else None
    )
    if dump_target:
    ...
    # Build the requested deck(s). Parsing above is format-independent, so a
    # single parse can emit one deck or, for "all", every deck. "json" builds
    # no apkg (the JSON dump above is its only output).
    if format == "all":
        formats_to_build = ["basic", "singlecloze", "onecloze", "multicloze"]
    elif format == "json":
        formats_to_build = []
    else:
        formats_to_build = [format]

    for fmt in formats_to_build:
        _write_deck(cards, media_files, fmt, _output_filename(fmt))
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=str,
        default="basic",
        choices=["basic", "singlecloze", "onecloze", "multicloze", "all", "json"],
        help=(
            "Card format to use: "
            ...
            "'all' to build every apkg plus cards_dump.json from one parse, "
            "'json' to only write the JSON dump (no apkg)."
        ),
    )
